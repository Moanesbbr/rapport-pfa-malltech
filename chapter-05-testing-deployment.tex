...
\textit{[CI/CD Pipeline Diagram]}\\
\small Test $\rightarrow$ Security $\rightarrow$ Build \& Push (GHCR) $\rightarrow$ Deploy (VPS) $\rightarrow$ Notify (Discord)\\
Deploy failure $\rightarrow$ Auto-rollback (re-tag previous image digest)
\vspace{2cm}}}
...
\begin{longtable}{|m{8cm}|m{4.5cm}|}
    ...
    \endlastfoot
    \hline
    Enable \texttt{forbidNonWhitelisted} on \texttt{Validation\-Pipe} & Pending \\
    \hline
    Configure \texttt{@nestjs/throttler} rate limiting & Pending \\
    \hline
    Set \texttt{secure: true} on session cookie & Pending \\
    \hline
    Audit and tighten CORS origins & Pending \\
    \hline
    Enable MongoDB authentication & Configured \\
    \hline
    Rotate all secrets (JWT, session, DB passwords) & Pre-launch \\
    \hline
    Set up automated \texttt{mongodump} backup strategy & Pending \\
    \hline
    \caption{Pre-production checklist}
    \label{tab:preprod}
\end{longtable}
